...
import { useComposedRefs } from "@/shared/lib/utils/compose-refs";
...
const animateLayoutChanges: AnimateLayoutChanges = (args) => defaultAnimateLayoutChanges({ ...args, wasDragging: false });
    ...
    <KanbanColumnContext.Provider value={columnContext}>
      <SortableContext items={items} strategy={context.orientation === "horizontal" ? horizontalListSortingStrategy : verticalListSortingStrategy}>
        <ColumnPrimitive
          ...
          style={composedStyle}
          className={cn(
            "flex size-full flex-col gap-2 rounded-lg border bg-muted/20 p-2.5 aria-disabled:pointer-events-none aria-disabled:opacity-50",
            {
