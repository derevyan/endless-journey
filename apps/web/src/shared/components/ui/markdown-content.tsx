...
const DEFAULT_PRE_BLOCK_CLASS = "my-4 overflow-x-auto w-fit rounded-xl bg-muted text-muted-foreground border border-border p-4";
...
const extractTextContent = (node: React.ReactNode): string => {
  ...
  if (isValidElement(node)) {
    // Safely access children prop with type guard
...
const components: Partial<Components> = {
  ...
  p: ({ children, ...props }: React.HTMLAttributes<HTMLParagraphElement>) => (
    <p className="leading-6 not-first:mt-4 break-all whitespace-pre-line" {...props}>
      {children}
    </p>
  ),
  ...
  u: ({ children, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <span className="underline underline-offset-2" {...props}>
      {children}
    </span>
  ),
  span: ({
    ...
    if (className?.includes("mention")) {
      return (
        <span
          className={cn("bg-primary text-primary-foreground rounded-xs px-2 py-0.5", className)}
          data-type={dataType}
          data-id={dataId}
  ...
  a: ({ children, ...props }: React.AnchorHTMLAttributes<HTMLAnchorElement>) => (
    <a className="font-medium underline underline-offset-4 whitespace-pre-wrap break-all" target="_blank" rel="noreferrer" {...props}>
      {children}
    </a>
  ),
  ...
  code: ({ children, node: _node, className, ...props }) => {
    ...
    return (
      <code className={cn("rounded bg-muted px-[0.3rem] py-[0.2rem] font-mono text-sm break-all", className)} {...props}>
        {children}
      </code>
    );
  },
